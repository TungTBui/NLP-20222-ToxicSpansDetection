...
import torch
from Model.BiLSTM_CRF.utils import PAD, OOV, load_json_file, save_json_file
...
class Preprocessor:
    ...
    def sent_to_vector(self, sentence, max_seq_len=0):
        ...
        max_seq_len = max_seq_len if max_seq_len > 0 else len(tokens)
        vec = [self.vocab_dict.get(c, self.OOV_IDX) for c in tokens[:max_seq_len]]
